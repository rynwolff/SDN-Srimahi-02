...
class M_Home extends CI_Model {
	...
	{
	return $this->db->order_by('id_program', 'DESC')->get('program', $limit, $start);
	}
	
	// public function get_program_by_id($id)
	// {
	// 	return $this->db->get_where('program', ['id_program' => $id]);
	// }
	...
	public function search_program($keyword)
{
    return $this->db
        ->like('nama_program', $keyword)
        ->or_like('deskripsi_program', $keyword)
        ->or_like('detail_program', $keyword)
        ->get('program')
        ->result_array();
}
